...
async function getEvent(slugOrId: string) {
    try {
        ...
        const eventBySlug = await (prisma as any).event.findUnique({
            where: { slug: slugOrId },
            include: {
        ...
        const eventById = await (prisma as any).event.findUnique({
            where: { id: slugOrId },
            include: {
                venue: { select: { id: true, name: true, address: true, mapUrl: true, city: true } },
                djs: {
...
export default async function EventPage(props: { params: Promise<{ city: string; slug: string }> }) {
    ...
    const jsonLd = {
        ...
        offers: {
            '@type': 'Offer',
            url: event.ticketUrl || `https://toocooltoparty.com/${params.city}/event/${event.slug || event.id}`,
                        ...
                        <ul style={{ listStyle: 'none', padding: 0 }}>
                            {event.djs.map(({ dj }: { dj: any }) => (
                                <li key={dj.id} style={{
                                    ...
                                    <a href={`/dj/${dj.id}`} style={{ textDecoration: 'none', color: 'white' }}>
                                        <span style={{ fontSize: 'var(--text-lg)', fontWeight: 600 }}>{dj.name}</span>
                                    </a>
